...
async function initTurso() {
    ...
    const r = await tursoClient.execute({ sql: "SELECT COUNT(*) as n FROM accounts", args: [] });
...
async function tursoLoadBucketsConfig() {
    ...
    const accounts = [];
    const accRes = await client.execute({ sql: "SELECT id, label, endpoint, access_key_id, secret_access_key FROM accounts", args: [] });
    ...
    const buckets = [];
    const bktRes = await client.execute({ sql: "SELECT id, account_id, bucket_name, label FROM buckets", args: [] });
    ...
    const setRes = await client.execute({ sql: "SELECT value FROM settings WHERE key = 'activeBucketId'", args: [] });
